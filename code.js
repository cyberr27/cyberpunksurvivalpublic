...
function reconnectWebSocket() {
  ...
  setTimeout(
    () => {
      ...
      fetch("https://cyberpunksurvivalserver.onrender.com/health")
        .then((response) => {
          if (!response.ok) {
            throw new Error("Сервер недоступен");
          }
          ws = new WebSocket("wss://cyberpunksurvivalserver.onrender.com");
          ws.onopen = () => {
            reconnectAttempts = 0;
            if (myId) {
  ...
  switch (data.type) {
    case "loginSuccess":
      ...
      const me = {
        ...
        lastXPAddTime: 0,
        lastDropTime: 0,
        lastVacuumRewardTime: 0,
        lastTwisterMsg: 0,
        twisterSpinning: false,
...
async function handleGameMessageLogic(data) {
  ...
  switch (data.type) {
    ...
    case "thimbleriggerMet":
      ...
      break;
    case "thimbleriggerStartGame": {
      // Обновляем инвентарь (ставка снята)
      const me = players.get(myId);
      if (me) {
        me.inventory = data.inventory.map((i) => (i ? { ...i } : null));
        window.inventorySystem.updateInventoryDisplay();
      ...
      // Запускаем анимацию игры (без знания правильной чашки)
      startSimpleGame(); // ← теперь без аргументов
      break;
    }
    case "thimbleriggerGameResultSync":
      if (data.success) {
        ...
        if (area && msgEl && data.correctCup !== undefined) {
          const cups = area.querySelectorAll(".thimble-cup");
          cups.forEach((c) => c.classList.add("lifted"));

          const ball = document.createElement("div");
          ball.className = "thimble-ball";
          const correctCupEl = cups[data.correctCup];
          if (correctCupEl) {
            ball.style.left = correctCupEl.style.left.replace("80px", "10px");
          } else {
            ball.style.left = "50%";
          }
          area.appendChild(ball);

          msgEl.textContent = data.won
            ? `ВЫИГРЫШ! ${data.bet * 2} баляров! +${data.xpGained} XP!`
            : "НЕ ПОВЕЗЛО!";
          msgEl.style.color = data.won ? "#00ff00" : "#ff0000";

          setTimeout(() => {
            // оставляем видимым результат
          }, 5000);
        }

        showNotification(
          data.won ? "Выигрыш зачислен!" : "Проигрыш",
          data.won ? "#00ff00" : "#ff5555",
        );

        lastSelectedCup = -1;
      } else {
        showNotification(data.error || "Ошибка игры", "#ff5555");
        lastSelectedCup = -1;
      }
      break;
