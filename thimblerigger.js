...
function startSimpleGame() {
  ...
  // Возвращаем старые, центрированные позиции шарика
  const ballPositions = [
    "calc(16.666% - 10px)",
    "calc(50% - 10px)",
    "calc(83.333% - 10px)",
  ...
  // Показываем шарик ровно под чашкой (как было раньше)
  const visualCup = Math.floor(Math.random() * 3); // ← только визуально
  const initialBall = document.createElement("div");
  initialBall.className = "thimble-ball";
  initialBall.style.left = ballPositions[visualCup];
